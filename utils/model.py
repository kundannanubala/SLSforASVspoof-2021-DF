...
from utils.config import DEFAULT_SSL_PATH


class SSLModel(nn.Module):
    def __init__(self, device, ssl_path=DEFAULT_SSL_PATH):
...
class Model(nn.Module):
    def __init__(self, args, device, ssl_path=DEFAULT_SSL_PATH):
        super().__init__()
        self.device = device
        self.ssl_model = SSLModel(self.device, ssl_path=ssl_path)
